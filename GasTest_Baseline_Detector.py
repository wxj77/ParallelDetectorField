...
printless = False
#printless = True
workdir ="/media/wei/ACA8-1ECD3/ComsolGeoFile/"
import sys
...
#LengthUnit="mm", ForceUnit="N", StressUnit="MPa",
GridList=[]
TopPlate = cf.LZ_Grid(GridName = "Top Plate",WireDiameter = 1465.*unit.m, WirePitch = 0.*unit.m, GridType = "P", z_Location = (110.5+8.125+X)*unit.m, Tension = 0.,GridDiameter = 146.*unit.m , MaterialMaxStress = 290.*unit.M)
#TopPlate.ElectricFieldUpInf = 0. # in V/mm
#TopPlate.ElectricFieldDownInf = .114e2  #in V/mm
#TopPlate.Print()
GridList.append(TopPlate)

AnodeGrid = cf.LZ_Grid(GridName = "Anode Grid",WireDiameter = 150.e-3*unit.m, WirePitch = 5*unit.m, GridType = "WW", z_Location = (8.125+X)*unit.m, Tension = 2.5,GridDiameter = 146.*unit.m , MaterialMaxStress = 290.*unit.M)
#AnodeGrid.ElectricFieldUpInf = .114e2 # in V/mm
#AnodeGrid.ElectricFieldDownInf = -10.2e2  #in V/mm
#AnodeGrid.Print()
GridList.append(AnodeGrid)
...
BottomPlate = cf.LZ_Grid(GridName = "Bottom Plate",WireDiameter = 1465.*unit.m, WirePitch = 0.*unit.m, GridType = "P", z_Location = (-4.875-110.5+X)*unit.m, Tension = 0.,GridDiameter = 146.*unit.m , MaterialMaxStress = 290.*unit.M)
#BottomPlate.ElectricFieldUpInf = 2.90e2 # in V/mm
#BottomPlate.ElectricFieldDownInf = .25e2  #in V/mm
#BottomPlate.Print()
GridList.append(BottomPlate)
...
VList=[-1.5e3,5.75e3,-5.75e3,-1.5e3,]
...
for a in np.arange(0,8.5,0.5):
    ...
    ss=ss+ '%.2f,'%((SYS_Gas.GridList[1].SurfaceFieldNaive()[0]/1.e5))   
...
for a in np.arange(0,8.5,0.5):
    VList=[-1.5e3,a*1e3,-a*1.e3,-1.5e3,]
    SYS_Gas.UpdateVoltage(VList)
    surface_voltage_list.append(SYS_Gas.GridList[2].SurfaceFieldNaive()/1.e5)
    ss=ss+ '%.2f,'%(-(SYS_Gas.GridList[2].SurfaceFieldNaive()[0]/1.e5))  
...
for a in np.arange(0,8.5,0.5):
    ...
    SYS_Gas.UpdateVoltage(VList)
    surface_voltage_list.append(SYS_Gas.GridList[2].SurfaceFieldNaive()/1.e5)
    ss=ss+ '%.2f,'%(-(SYS_Gas.GridList[1].ElectricFieldDownInf/1.e5))
#    print '%.2f,'%(-(SYS_Gas.GridList[1].ElectricFieldDownInf/1.e5))
...
surface_voltage_list=[]
for a in np.arange(0,8.5,0.5):
    VList=[-1.5e3,a*1e3,-a*1.e3,0,0,-1.5e3,]
    SYS_Liq.UpdateVoltage(VList)
...
for a in np.arange(0,8.5,0.5):
    ...
    SYS_Liq.UpdateVoltage(VList)
    surface_voltage_list.append(SYS_Liq.GridList[2].SurfaceFieldNaive()/1.e5)
    ss=ss+ '%.2f,'%(-(SYS_Liq.GridList[1].ElectricFieldDownInf/1.e5))
#    print '%.2f,'%(-(SYS_Liq.GridList[1].ElectricFieldDownInf/1.e5))
